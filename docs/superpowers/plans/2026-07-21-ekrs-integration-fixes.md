...
def test_host_allowlist_blocks_when_set_and_mismatch(monkeypatch):
    ...
    """
    monkeypatch.setenv("CALLBACK_ALLOWED_SCHEMES", "https")
    monkeypatch.setenv("CALLBACK_ALLOWED_HOSTS", "parser.internal.example.com")
    with pytest.raises(CallbackURLBlockedError, match="not in allowlist"):
        validate_callback_url("https://attacker.example.com/cb")


def test_host_allowlist_accepts_known_host(monkeypatch):
    monkeypatch.setenv("CALLBACK_ALLOWED_SCHEMES", "https")
    monkeypatch.setenv(
        "CALLBACK_ALLOWED_HOSTS", "parser.internal.example.com,parser.dev",
    )
    parsed = validate_callback_url("https://parser.internal.example.com/cb")
    assert parsed.host == "parser.internal.example.com"
...
def test_host_allowlist_wildcard_allows_any(monkeypatch):
    ...
    parsed = validate_callback_url("https://anything.example.com/cb")
    assert parsed.host == "anything.example.com"
...
def test_trailing_dot_host_normalized(monkeypatch):
    ...
    D3 fix: `https://parser.example.com./cb` host is "parser.example.com."
    which would mismatch the allowlist "parser.example.com" without this fix.
    """
    monkeypatch.setenv("CALLBACK_ALLOWED_SCHEMES", "https")
    monkeypatch.setenv(
        "CALLBACK_ALLOWED_HOSTS", "parser.example.com",
    )
    parsed = validate_callback_url("https://parser.example.com./cb")
    assert parsed.host == "parser.example.com"  # stripped
```
...
# D4: callback outcome counters for observability. Phase 5 added a metrics
# registry; this reuses the prometheus_client Counter directly. Match the
# existing helper if `ekrs_rag.core.metrics` exposes one.
CALLBACK_OUTCOMES = Counter(
    "rag_callback_total",
    "RAG callback outcomes (one per terminal branch of _send_callback)",
    ["outcome"],
)
# outcome values: sent, url_blocked, auth_missing, nonretryable_4xx

# D5: audit events for callback terminal outcomes. Reuses Phase 5's
# AuditWriter; pipeline receives the writer in __init__ (DI'd from lifespan).
# If no writer is wired, AuditWriter is graceful no-op (Phase 5 contract).
```
...
Replace `pipeline.py:30-95` body so each branch returns an `IngestionOutcome` and `_send_callback` is still invoked before the return. Pattern (E1 helper: two small methods collapse the 5 duplicated branch tails):
...
# Callback exception classes are defined alongside _send_callback (T6);
# ingest() never raises on callback transport failures.
```

Note: collapsed 5 duplicated branch tails to one `_failed_outcome` call + one `_send_callback_safely` await each. `_send_callback_safely` is the E1/D1/D5 merge point — D1 inline-try/except, D5 audit emit, plus the actual `_send_callback` call all live there.
...
Expected: tests that called `await pipeline.ingest(...)` and ignored the return value still pass; tests that asserted on exception behavior may need updates (those are not yet written). **E5 caveat**: if a test uses `pytest.raises(Exception)` around `await pipeline.ingest(...)`, the test now passes the exception through `_send_callback_safely` and only the unhandled-exception branch of `_run_locked_ingest` will surface it. Audit `tests/integration/test_ingestion.py` and `tests/integration/test_ingestion_replay.py` for such usages before declaring PASS — update them to assert on `outcome.rag_status == "failed"` and `outcome.error_code` instead.
...
**Files:**
- Modify: `EKRS-RAG-AI_intergration.md` (locate passages by content)
- Modify: `docs/USAGE.md` (add `.ready` clause)
- Modify: `CHANGELOG.md` (append entry)

- [ ] **Step 1: Locate the false claims by content (survives line drift)**

```bash
grep -nE 'RAG 服务开始处理的唯一信号|扫描 /parsed_lib/\*/\{timestamp\}/\.ready|\.processed' \
  EKRS-RAG-AI_intergration.md
```

Expected: three match sites — one for the "唯一信号" claim, one for the "扫描 ... .ready" claim, one for the ".processed" rename. If grep returns no matches, the doc has already been fixed and this task is a no-op (skip to Step 4).

- [ ] **Step 2: Read context around each match**

For each match line returned by Step 1, run `sed -n "<line-3>,<line+3>p" EKRS-RAG-AI_intergration.md` to read the surrounding paragraph before editing.

- [ ] **Step 3: Replace the three false passages**

In `EKRS-RAG-AI_intergration.md`:

For the "RAG 服务开始处理的唯一信号" passage, replace with:

> 备注：`.ready` 文件由 parser 原子创建，作为 parser 侧的发布完成信号。RAG 服务**不读取** `.ready`；parser 必须在 JSONL 完整落盘、fsync 完成后才发送 notify。

For the "扫描 /parsed_lib/*/{timestamp}/.ready，对每个 .ready 文件..." passage, replace with:

> 备注：RAG 没有 `.ready` 轮询扫描器；本说明遗留自早期设计，与当前实现不符。RAG 完全依赖 notify HTTP 触发。

Remove the `.processed` rename instructions entirely.

- [ ] **Step 4: Add `.ready` clause to `docs/USAGE.md`**
...
- [ ] **Step 5: Append `CHANGELOG.md` entry**
