...
Phase 10 T10a-1	broad-spectrum retrieval 第 1 步: rag/ekrs_rag/retrieval/fts_manager.py (SQLite FTS5 镜像 Qdrant payload, schema 7 列 + unicode61 remove_diacritics 2 tokenizer + BM25 归一化 |bm25|/(1+|bm25|) + delete_by_chunk_id 单 chunk 回滚原语 + payload_json UNINDEXED 防 JSON key tokenize + generate_chunk_id 双向映射生成器) + 22 unit + 8 integration test	keyword BM25 召回通道就位 (engineering identifier + 短语精确匹配)	30/30 pass, mypy 干净; 路径 `/app/rag/fts.sqlite`, sync sqlite3 + check_same_thread=False; CJK 召回差是已知限制 (unicode61 不拆 CJK run, jieba 是 follow-up, 不在 T10a-1); T10a-2 (pipeline 接入) / T10a-3 (RRF) / T10a-4 (retriever 并联) / T10a-5 (chunk_id 时机) / T10a-6 (golden) / T10a-7 (audit) 未做
Phase 10 T10a-2	broad-spectrum retrieval 第 2 步: pipeline.ingest() Step 5.6 同步写 FTS (Qdrant 成功后) + replace_doc 原子 delete+upsert (re-ingest 不重复) + FTSManager.count_active + QdrantManager.count_points + ConsistencyChecker 5min 对账 (detect-only, 不修复) + fts_consistency_drift 审计 schema 注册 (event count 19→20)	detect-only 漂移告警 + pipeline FTS 镜像同步就位	10 unit + 5 integration = 15 pass; mypy 干净; 完整 suite 0 退化; pipeline 注入 `fts=None` kwarg 保持 Phase 9 baseline; T10a-3 (RRF) / T10a-4 (retriever 并联) / T10a-5 (chunk_id 时机) / T10a-6 (golden) / T10a-7 (audit fts_synced) 未做
Phase 10 T10a-3	broad-spectrum retrieval 第 3 步: rag/ekrs_rag/retrieval/rank_fusion.py 纯函数 RRF (no I/O, R2 纯函数确定性) + FusionStats frozen dataclass (vector_hits/fts_hits/both_hits) 直接供 T10a-7 audit 字段用, retriever 不重复算	dual-list RRF 通路就位 (FusionStats 字段已就绪, T10a-7 audit emit 直接取用)	17 unit pass; mypy 干净; 0 退化; k=60 硬编码默认 (parent plan §cross-doc adjud 锁定), 测试用 k=10/k=1 验证逻辑; N=2 主路径 (vector + FTS) + N=1 退化 + N=3 forward-compat; tie-break = insertion-order (deterministic R2); T10a-4 (retriever 并联接入) / T10a-5 (chunk_id 时机) / T10a-6 (golden) / T10a-7 (audit fts_searched) 未做
Phase 10 T10a-4	broad-spectrum retrieval 第 4 步: EKRSRetriever `async def retrieve()` + `fts: FTSManager | None = None` kwarg + asyncio.gather 并联 vector+FTS (return_exceptions=True 隔离 FTS 异常, log warning, 降级到 vector-only) + FTSManager.search_with_payload 单次 IN-查询避免 N+1 + RetrievalResult.fusion_stats 字段 (None=fts 关闭) + scope_priority 仍在 RRF 之后 (R4) + constraints.py:147/208 await + _StubRetriever/AsyncMock 迁移	retriever RRF 就位, FTS 异常静默降级, Phase 9 退化路径 byte-level 不变	10 unit + 3 IMPROVE boundary + 4 Phase 6B regression + 1 FTSManager search_with_payload = 18 pass; 完整 suite 800 pass 0 退化; mypy 干净 (1 NEW error @ retriever.py:73 attr-defined 已 patch); T10a-5 (chunk_id 时机) / T10a-6 (golden) / T10a-7 (audit fts_searched emit) 未做; 无新 tag (phase10.1 锁 1c44eee, phase10 留给 T10a-7 closure)
Phase 10 T10a-5	broad-spectrum retrieval 第 5 步: chunk_id EKRS 侧生成 + 双向映射 (FTS↔Qdrant). `QdrantManager.upsert_chunks` 写 payload `chunk_id={doc_hash[:8]}-{idx:04d}`; `FTSManager.get_block_id_by_chunk_id` 新方法 (T10a-1 `get_chunk_id` 互补反向); Chunk model 加 `chunk_id: Optional[str] = None`; retriever key_fn `c.chunk_id or fallback` 双轨 (新 doc 走 chunk_id, legacy 走 doc_hash:block_id). 命名空间共存 (parent §[M2]): `block_id` (UUID) 与 `source_block_ids` (list) 保留, `chunk_id` 是并行字段, 不替换	chunk_id 双向 round-trip 就位, retriever key 稳定性提升 (chunk_id 是 doc-scoped 唯一, fallback 仅 legacy doc)	7 unit + 3 IMPROVE boundary + 3 FTS bidirectional = 13 pass; 完整 suite 812 pass 0 退化; mypy 干净; T10a-6 (golden) / T10a-7 (audit fts_synced/fts_searched emit, event count 20→22) 未做; 无新 tag (phase10 留给 T10a-7 closure)
Phase 10 T10a-6	broad-spectrum retrieval 第 6 步: golden set 50 case 回归验证 + 3 工程标识符 BM25-only recall@1 决策数据收集 (验证阶段, 不扩 case). `tests/golden_set/` 208 pass (50 case 含参数化); `tests/unit/test_fts_identifier_recall.py` 4 测试 soft-assertion + stdout log. **结果: 3/3 recall@1=1** — unicode61+remove_diacritics tokenizer 对 Latin+digit+连字符/斜杠/点 标识符召回干净; CJK run 是已知限制. T10c cross-encoder 触发条件 (parent §6.1): 决策数据 3/3 满, 不强制触发	BM25 标识符召回基线就位, 50 case 回归通过, T10c 决策依据已收集	208 golden + 4 BM25 identifier = 212 pass; 完整 suite 816 pass 0 退化; mypy 干净; T10a-7 (audit fts_synced/fts_searched emit, event count 20→22) 未做; 无新 tag (phase10 留给 T10a-7 closure)
Phase 10 T10a-7	broad-spectrum retrieval 第 7 步 (closure): 审计事件 `fts_synced` + `fts_searched` 注册到 `main.py _EVENT_SCHEMAS` (event count 20→22)。`EKRSRetriever` 加 `audit_writer: Optional[AuditWriter] = None` kwarg (Phase 5.5 E DI 模式); `fts=None`/`audit_writer=None` 双默认保留 Phase 9 byte-level baseline. retriever 在 RRF 完成后 emit `fts_searched` (读 FusionStats 三字段), best-effort (`try/except` 隔离, 审计失败不阻塞业务 — parent §204). `main.py` lifespan inject `_audit_writer` 到 retriever. **IngestionOutcome enum 不增字段** (parent §204 明示关闭; 中间步骤非最终态). 14 unit test + 2 Phase 6A regression (count 20→22, names 加 `fts_synced`/`fts_searched`)	审计回放确定性闭合; FTS 写入 + 检索全程可回放; `phase10` tag force-move 到本任务闭合 commit (parent §111)	14 t10a7 + 2 phase6a-reg = 16 pass; 完整 suite 622 pass + 1 skip; mypy 干净 (无 T10a-7 NEW error); CHANGELOG `[phase10]` release section + version bump 0.0.5→0.1.0; **`phase10` annotated tag FF push 闭合 Phase 10**
Phase 10 T10b-3	broad-spectrum retrieval 第 8 步 (incremental): 强信号短路 (exact-match short-circuit). 当用户 query 是任何 retrieved chunk 的 `chunk.text` 子串时, retriever 旁路 RRF 直接返回匹配 chunk, `vector_scores=[1.0]`, `fusion_stats=FusionStats(N,0,0)`. 全局启用, 不门控 strict 模式 (parent §25+§157: 短路是确定性优化, 不是 strict-mode 推断). 短路 gated on `fts is not None` (preserves Phase 9 byte-level baseline). `RetrievalResult` 加 `short_circuit: bool = False` field (T10b-3 predicate 与 R4 scope_priority 在新路径仍生效). 新 `EKRSRetriever._is_exact_match(query, chunks) -> List[int]` static predicate (case-sensitive substring, empty query → `[]`). `fts_searched` audit emit 仍 emit (运营可见性). 测试: 11 unit (predicate 5 + retriever 4 + scope/strict parity 2); 完整 suite 633 unit + 208 golden + 11 t10b3 = 852 pass 0 退化; mypy 干净. Bench: `scripts/t10b3_short_circuit_bench.py` stub 测试, sc_p99 3.88ms vs rrf_p99 4.45ms (ratio 0.87 < 0.99 acceptance; plan-doc aspirational 0.5 留待真实 bge-m3+FTS5+Qdrant 后端验证). **T10b-3 是 `phase10` 闭合 commit 后的 incremental — 无新 tag (`phase10` 已锁在 `2e1d9fa`, `phase10.1` 在 `1c44eee` T10b-1 do-not-move)**	短路命中率作为查询画像的可观察信号 (Prometheus counter 留 Phase 11); 短路路径 fts_synced/fts_searched audit 不变; 任何 future 跨 chunk 级别的"compound 短路" 是 T10b follow-up 候选		11 new test pass 0 退化; full suite +11 = 633 unit; mypy 干净; **无新 tag**
Phase 10 T10d Td.1	broad-spectrum retrieval 第 9 步 (incremental): MCP (Model Context Protocol) 适配层最小可行. 新 `rag/ekrs_rag/mcp/server.py` 模块, 用官方 Python `mcp>=1.0` SDK (`FastMCP` high-level API) 暴露 2 工具: `ekrs_search(query, top_k=40, active_scope=None)` 内部直接调 `EKRSRetriever.retrieve()` (无内部 HTTP, 无 double rate-limit, 无 double audit — T10a-7 `fts_searched` audit 复用) + `ekrs_status()` 返 healthz payload 不依赖 retriever (server 启动优先). JSON-over-TextContent MCP wire format; 异常隔离 (parent §204: retriever raise → `{"error": "..."}` MCP content 不 crash server). chunk.text 截断 200 字 (`CHUNK_TEXT_PREVIEW_CHARS`). stdio 单 transport (Claude Code + MCP inspector + Desktop 都支持; Streamable HTTP 是 PoC 后期项). CLI: `python -m ekrs_rag.mcp.server`. 测试: 8 unit (`tests/unit/test_mcp_server_td1.py` — module imports + tool 注册 + dispatch kwargs + JSON TextContent 输出 + 异常隔离 + 空 chunks + status 独立) + 1 integration (`tests/integration/test_mcp_stdio_roundtrip_td1.py` — 子进程 round-trip initialize + list_tools + call_tool ekrs_status + call_tool ekrs_search error-path). pyproject 加 `mcp>=1.0` [project.dependencies] (生产 dep, 非 dev-only) + `integration` pytest mark. 完整 suite **849 unit + 1 stdio integration + 1 skip pass 0 退化**; mypy `ekrs_rag/mcp/` 干净 (1 dict-item 错误用 `Dict[str, Any]` 显式注解闭合). **Td.1 是 `phase10` 闭合后 incremental — 无新 tag (`phase10` 锁 `2e1d9fa`, `phase10.1` 锁 `1c44eee`)**	Td.2 (extend 到 `ekrs_query` + `ekrs_get_block`) 推迟至 MCP 消费方证需求 (Claude Code 集成 or external Agent); Td.3 (`.mcp.json` 配置 + Claude Code 实跑) 视 Td.2 决定; 跨进程 retriever 复用 (stdio 独立进程 vs in-process FastAPI) 是 PoC 后期评估项	8 new unit + 1 integration pass; full suite +8 = 849 unit; mypy 干净; **无新 tag**
Phase 10 T10d Td.2	broad-spectrum retrieval 第 10 步 (incremental): MCP 适配层扩展, 加 2 工具 + 1 HTTP route + 1 helper extractor. `ekrs_query(query, context, scope, policy, overlay_hints, strict, top_k)` 内部直接调 `evaluate_constraints(retriever, ...)` helper (从 constraints 路由 handler 抽出, 单一 source of truth, 无内部 HTTP — 避免 double rate-limit + double audit), 完整 R3 三闸门 + R4 scope + R6 strict + R7 scope_path 全部透明透传 (solver 是 R2 纯函数); 异常隔离 (parent §204: solver raise / 404 / 409 / 503 全部 → `{"error": "..."}` MCP content). `ekrs_get_block(block_id)` 内部直接调 `QdrantManager.get_payload_by_block_id(block_id)` (scroll + filter, 复用 `get_ingestion_status` lines 274-289 模式, limit=1 利用 UUID 唯一性), 返回完整 block payload (text NOT 截断; 是 document 精读, 不是 search preview); `numeric_hints` 投影为 count-only (避免 MCP message-size 限制); 404 → `{"error": "block_id not found"}` MCP content. **新 HTTP route** `GET /v1/blocks/{block_id}` (新 `rag/ekrs_rag/api/routes/blocks.py` 模块, `require_parser_token` auth 跟 `/v1/constraints` 一致, `BlockResponse` Pydantic 模型, 404/503/500 三态, qdrant 异常隔离), `main.py` lifespan 注册. **Naming 统一 (user feedback)**: 用 `block_id` (UUID) 统一新 route param + MCP tool param, 跟 FTS5 PK + Qdrant payload + 审计事件命名一致; T10a-5 `chunk_id={doc_hash[:8]}-{idx:04d}` 作为并行字段保留 (parent §[M2] 命名空间共存规约), 不替换. **`build_server` DI 扩展** 从 2 args `(retriever, dependencies)` → 4 args `(retriever, qdrant, solver, dependencies)`, closure capture 保留 (Phase 5.5 E DI 模式), 不引全局. CLI entrypoint 仍传 None (PoC zero-config, Td.3 才是 production wiring — Claude Code `.mcp.json` 集成). 测试: 9 mcp unit (`tests/unit/test_mcp_server_td2.py` — 4 imports/build_server + 4 ekrs_query + 3 ekrs_get_block) + 4 blocks-route unit (`tests/unit/test_blocks_route.py` — happy path 200 + 404 + 503 + 500 异常隔离) + 1 stdio integration (`tests/integration/test_mcp_stdio_roundtrip_td2.py` — 4 工具 stdio round-trip). Td.1 `test_build_server_registers_two_named_tools` + Td.1 stdio roundtrip 同步放宽到 "at least 2 tools" contract (Td.2 的 registry 扩展让"exactly 2"失效). 0 退化: 656 unit pass + 1 skip; 完整 stdio integration 2 pass; mypy 干净 (`TextContent` union narrowing 用 `_as_text(content_block) -> TextContent` helper 在两个 stdio roundtrip 测试文件). 10 个 Phase 5/7 integration 失败 (`await RetrievalResult` 在 test_query_replay + test_constraints_api 的旧 sync stub 上) 经 `git stash` round-trip 验证为 pre-existing baseline failure, 跟 Td.2 无关. **Td.2 是 `phase10` 闭合后 incremental — 无新 tag (`phase10` 锁 `2e1d9fa`, `phase10.1` 锁 `1c44eee`)**	Td.3 (`.mcp.json` 配置 + Claude Code / Desktop 实跑) 是唯一未做项; production wiring (real retriever+qdrant+solver 注入 `build_server` + 启动脚本) 留 Td.3. 命名空间规约 (`block_id` UUID + `chunk_id` T10a-5 stable 并行字段 + `source_block_ids` list) 现在三 surface (HTTP / MCP / FTS5) 全部对齐, 旧 legacy chunk 通过 `ekrs_search` 短路路径仍可达 (T10b-3)	14 new test (9 mcp + 4 route + 1 integration) pass 0 退化; full suite 656 unit + 2 stdio + 1 skip = 659; mypy 干净; **无新 tag**
